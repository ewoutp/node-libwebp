...
static int SaveOutput(const WebPDecBuffer* const buffer,
  ...
  if (format == PNG) {
    ...
    ok &= WriteJPEG(fout, buffer, jpeg_quality);
  } else if (format == PAM) {
...
int main(int argc, const char *argv[]) {
  ...
  for (c = 1; c < argc; ++c) {
    if (!strcmp(argv[c], "-h") || !strcmp(argv[c], "-help")) {
      ...
      format = TIFF;
    } else if ((!strcmp(argv[c], "-jpg") || !strcmp(argv[c], "-jpeg")) && c < argc - 1) {
      format = JPEG;
      jpeg_quality = strtol(argv[++c], NULL, 0);
...

  {
    ...
    switch (format) {
      ...
      case PAM:
        ...
        break;
      case JPEG:
        output_buffer->colorspace = MODE_RGB;  // drops alpha for jpeg
        break;
      case PPM:
