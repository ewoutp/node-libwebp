...
VP8StatusCode WebPAllocateDecBuffer(int w, int h,
                                     WebPDecoderOptions* const options,
  ...
  if (options != NULL) {    // First, apply options if there is any.
    ...
    if (options->use_scaling) {
      if (options->scaled_width <= 0 && options->scaled_height <= 0) {
        return VP8_STATUS_INVALID_PARAM;
      } else if(options->scaled_width <= 0) {
      	  double aspect = (double)w / (double)h;
		  h = options->scaled_height;
		  w = (int)(0.5 + (double)options->scaled_height * aspect);
		  options->scaled_width = w;
		  options->scaled_height = h;
	  } else if(options->scaled_height <= 0) {
      	  double aspect_inv = (double)h / (double)w;
		  w = options->scaled_width;
		  h = (int)(0.5 + (double)options->scaled_width * aspect_inv);
		  options->scaled_width = w;
		  options->scaled_height = h;
	  } else {
		  w = options->scaled_width;
